...
import { WORLD_HEX } from "@/lib/world-core/world-colors";
import type { WorldCoreSceneHandle, WorldState } from "@/lib/world-core/types";
import { WORLD_CORE_MAX_DPR, WORLD_CORE_MOBILE_MAX_DPR, WORLD_CORE_TARGET_FPS } from "@/lib/world-core/eligibility";
...
export async function createWorldCoreScene(
  ...
  let lastFrame = 0;
  let last = 0;
  let currentWorld: WorldState = "core";
  ...
  const lerpPreset = (dt: number) => {
    ...
    ringMats.forEach((m) => {
      m.emissiveIntensity = currentPreset.emissiveIntensity * 0.9;
    });
  ...
  const resize = (width: number, height: number) => {
    ...
    const isMobile = options.mobile ?? width < 1024;
    const dpr = Math.min(
      window.devicePixelRatio || 1,
      isMobile ? WORLD_CORE_MOBILE_MAX_DPR : WORLD_CORE_MAX_DPR,
    );
  ...
  const setWorldState = (world: WorldState, immediate = false) => {
    currentWorld = world;
    targetColor.set(WORLD_HEX[world]);
